...
@Autonomous(name = "State: WITH WAIT Red Diagonal", group = "State")
//@Disabled
public class Auto_red_diagonal_with_WAIT extends OpMode {
  ...
  public void init() {
    ...
    side_right.setDirection(DcMotor.Direction.REVERSE);
  ...
  public void loop() {
    ...
    switch (mRobotState){
      ...
      case STATE_SCOOT_BACK:
        if (mStateTime.time() > iSearchTime){
          telemetry.addData("SCOOT BACK", "BACK");
            scoot_back();
          if (mGoToCenter){
            telemetry.addData("within scoot back", "Drive to Center");
            newState(State.STATE_DRIVE_TO_CENTER);
          }
          else{
            telemetry.addData("within scoot back", "Drive to Beacon 2");
      ...
      case STATE_DRIVE_TO_BEACON_TWO:
        telemetry.addData("drive to", "BEACON 2");
        if (mStateTime.time() > .50) {
            up_left.setPower(0.0);
            up_right.setPower(0.0);
            mGoToCenter = true;
            mShifted = false;

          telemetry.addData("drive_to", "beacon_2", side_right.getCurrentPosition());
            mVerified = false; //resets to false - to verify 2nd beacon
            run_to_position_Encoders();
            int b2COUNTS = myUtils.getNewCounts(106); //AKA 48" - 1/14/17 107 originial
            if (mShifted == true){
               b2COUNTS = myUtils.getNewCounts(100); //if goes into STATE SEARCH original = 101
            }
            telemetry.addData("drive_to", "beacon_2", side_left.getCurrentPosition());
            telemetry.addData("B2 COUNTS", b2COUNTS);
            side_right.setTargetPosition(b2COUNTS);
            side_left.setTargetPosition(b2COUNTS);
            side_left.setPower(0.8);
            side_right.setPower(0.8);
          //increased power from 0.6 to 0.8 1/8/17

          if (side_right.getCurrentPosition() >= b2COUNTS) {
            telemetry.addData("reached", "beacon_2", side_right.getCurrentPosition());
            side_left.setPower(0);
            side_right.setPower(0);
            newState(State.STATE_DETECT_COLOR);
          }
